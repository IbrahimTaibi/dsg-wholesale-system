      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <h1 className="text-3xl sm:text-4xl font-bold bg-gradient-to-r from-orange-600 to-yellow-600 bg-clip-text text-transparent">
                {t("premiumJuices")}
              </h1>
              <p className="text-gray-600 dark:text-gray-300 mt-1">
                {t("natural100")} • {filteredProducts.length}{" "}
                {t("productsAvailable")}
              </p>
            </div>
          </div>
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 dark:border-gray-700/50 p-12 text-center">
                ...
                <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-4">
                  {searchQuery ? "No juices found" : "No juices available"}
                </h2>
                <p className="text-gray-600 dark:text-gray-300 max-w-md mx-auto">
                  {searchQuery
                    ? `We couldn't find any juices matching "${searchQuery}". Try adjusting your search or filters.`
                    : "Check back soon for fresh natural juices!"}
                </p>
              </div>
            ) : (
              <div className="space-y-6">
                ...
                {pagination && pagination.pages > 1 && (
                  <div className="text-center pt-8">
                    <button className="px-8 py-4 bg-gradient-to-r from-orange-500 to-yellow-500 text-white rounded-xl hover:from-orange-600 hover:to-yellow-600 transition-all duration-200 font-semibold shadow-lg hover:shadow-xl transform hover:-translate-y-0.5">
                      Load More Juices
                    </button>
                  </div>
                )}
